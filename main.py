...
GAME_DATA = {
    ...
    "cardDatabase": [
        {"id": "woa-001","tier": "Common","name": "Goblin Skirmisher","atk": 1,"def": 1,"hp": 3},
        {"id": "woa-002","tier": "Common","name": "Orc Archer","atk": 2,"def": 1,"hp": 3},
        {"id": "woa-003","tier": "Common","name": "Gondor Footman","atk": 2,"def": 2,"hp": 4},
        {"id": "woa-004","tier": "Common","name": "Lórien Lookout","atk": 2,"def": 2,"hp": 3},
        {"id": "woa-005","tier": "Common","name": "Elven Scout","atk": 3,"def": 1,"hp": 3},
        {"id": "woa-006","tier": "Common","name": "Dwarven Miner","atk": 1,"def": 3,"hp": 5},
        {"id": "woa-007","tier": "Common","name": "Rohan Stablehand","atk": 1,"def": 2,"hp": 4},
        {"id": "woa-008","tier": "Common","name": "Misty Mountain Warg","atk": 3,"def": 1,"hp": 4},
        {"id": "woa-009","tier": "Common","name": "Hill-troll Runt","atk": 3,"def": 2,"hp": 5},
        {"id": "woa-010","tier": "Common","name": "Shadow-spawn","atk": 2,"def": 1,"hp": 3},
        {"id": "woa-011","tier": "Uncommon","name": "Rivendell Bladesman","atk": 4,"def": 3,"hp": 6},
        {"id": "woa-012","tier": "Uncommon","name": "Knight of the White Tower","atk": 4,"def": 4,"hp": 6},
        {"id": "woa-013","tier": "Uncommon","name": "Mirkwood Ranger","atk": 5,"def": 2,"hp": 5},
        {"id": "woa-014","tier": "Uncommon","name": "Iron Hills Veteran","atk": 3,"def": 5,"hp": 7},
        {"id": "woa-015","tier": "Uncommon","name": "Uruk-hai Captain","atk": 5,"def": 3,"hp": 6},
        {"id": "woa-016","tier": "Uncommon","name": "Barrow-wight","atk": 4,"def": 2,"hp": 6},
        {"id": "woa-017","tier": "Uncommon","name": "Haradrim Serpent-Caller","atk": 4,"def": 3,"hp": 5},
        {"id": "woa-018","tier": "Uncommon","name": "Watcher in the Woods","atk": 3,"def": 3,"hp": 7},
        {"id": "woa-019","tier": "Uncommon","name": "Stone-giant Thrower","atk": 5,"def": 2,"hp": 7},
        {"id": "woa-020","tier": "Uncommon","name": "Eregion Forgemaster","atk": 2,"def": 4,"hp": 5},
        {"id": "woa-021","tier": "Rare","name": "Captain of Gondor","atk": 6,"def": 5,"hp": 8},
        {"id": "woa-022","tier": "Rare","name": "Elven Lord of the West","atk": 7,"def": 4,"hp": 7},
        {"id": "woa-023","tier": "Rare","name": "Dwarven Runekeeper","atk": 5,"def": 6,"hp": 9},
        {"id": "woa-024","tier": "Rare","name": "Cavern Drake","atk": 7,"def": 3,"hp": 8},
        {"id": "woa-025","tier": "Rare","name": "Black Uruk Commander","atk": 7,"def": 4,"hp": 8},
        {"id": "woa-026","tier": "Rare","name": "Mountain Troll Chieftain","atk": 6,"def": 5,"hp": 9},
        {"id": "woa-027","tier": "Rare","name": "Shadow Wraith","atk": 6,"def": 4,"hp": 7},
        {"id": "woa-028","tier": "Rare","name": "Eagle of the Peaks","atk": 5,"def": 3,"hp": 7},
        {"id": "woa-029","tier": "Legendary","name": "Ur-Ghash, the Mountain's Heart","atk": 9,"def": 7,"hp": 12},
        {"id": "woa-030","tier": "Legendary","name": "Thraknar, the Black Calamity","atk": 10,"def": 6,"hp": 11}
    ]
}
...
class Player:
    ...
    def get_attacking_cards(self):
        ...
        return [card for card in self.field if card is not None and card.position == "ATK" and card.can_attack]
...
class GameState:
    ...
    def get_all_possible_moves(self):
        ...
        if self.phase == "DRAW":
                ...
                for i, card_in_hand in enumerate(current_player.hand):
                    for j, field_slot in enumerate(current_player.field):
                        if field_slot is None:
                            ...
                            moves.append((f"PLAY {card_to_play.name} in ATK slot {j+1}", state_atk))
                            ...
                            moves.append((f"PLAY {card_to_play_def.name} in DEF slot {j+1}", state_def))
            ...
            state_end.current_turn_player = "p2" if self.current_turn_player == "p1" else "p1"
    ...
    def perform_attack(self, attacker_slot_idx, target_slot_idx):
        ...
        if damage > 0:
            print(f"  > {attacker_card.name} attacks {target_card.name} for {damage} damage!")
            ...
            print(f"  > {attacker_card.name} attacks {target_card.name}, but DEF holds!")
    ...
    def perform_direct_attack(self, attacker_slot_idx):
        ...
        print(f"  > {attacker_card.name} attacks {target_player.name} directly for {damage} damage!")
...
class Game:
    ...
    def get_human_move(self):
        ...
        if self.game_state.phase == "DRAW":
            ...
            return ("DRAW", self.game_state.get_all_possible_moves()[0][1]) # Only one draw move
            ...
            while True:
                ...
                for i, card in enumerate(player.hand):
                    print(f"  {i+1}: {card.name} ({card.base_atk}/{card.base_def}/{card.base_hp})")
                print("\nOptions:")
                print("  play [hand_num] [field_slot] [atk/def]  (e.g., 'play 1 1 atk')")
                print("  pass")
                
                choice = input("> ").lower().strip()
                
                if choice == "pass":
                    for move_desc, next_state in self.game_state.get_all_possible_moves():
                ...
                if choice.startswith("play"):
                    ...
                    if len(parts) == 4:
                        try:
                            ...
                            for desc, next_state in self.game_state.get_all_possible_moves():
                                if desc == move_desc:
                    ...
                    try:
                        ...
                        if parts[2] == "player":
                            ...
                            for desc, next_state in self.game_state.get_all_possible_moves():
                                if desc == move_desc:
                                    return (desc, next_state)
                            print("You can't attack the player directly (field is not empty).")
                        
                        else:
                            # Card attack
                            target_idx = int(parts[2]) - 1
                            targets = list(enumerate(self.game_state.get_opponent_player().get_attackable_cards()))
                            ...
                            move_desc = f"ATTACK {attacker_card.name} -> {target_card.name}"
                            for desc, next_state in self.game_state.get_all_possible_moves():
                                if desc == move_desc:
                                    return (desc, next_state)
                            print("Error finding that attack move.") # Should not happen
